...
public class InsBoardService {
	...
	public int boardDelete(long boardNum) {
		...
		try {
			insBoardDao.boardFileDelete(boardNum);
			insBoardDao.boardLikeDelete(boardNum);
			insBoardDao.commentDelete(boardNum);
			if(insBoardDao.boardDelete(boardNum) > 0) {
				result = 1;
			}
		}catch(Exception e) {
			logger.error("[InsBoardService] boardDelete -- ", e);
		}
	...
	//댓글 좋아요 등록여부 및 본인확인
	public int myCommentLike(InsCommentLike insCommentLike) {
		int cnt = 0;
		
		try {
			cnt = insBoardDao.commentCnt(cnt);
		}catch(Exception e) {
			logger.error("[InsBoardService] myCommentLike -- ", e);
		}
